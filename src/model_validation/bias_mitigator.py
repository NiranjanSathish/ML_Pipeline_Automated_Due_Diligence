...
class BiasMitigator:
    ...
    def adjust_scores_by_metadata(self, chunks: List[Dict]) -> List[Dict]:
        """
        Iterate through chunks and apply boost if chunk metadata (sector, market_cap) 
        matches a biased/underperforming group in the report.
        """
        if not self.bias_map:
            return chunks
            
        for chunk in chunks:
            # Metadata might be in 'metadata' dict or flat fields
            meta = chunk.get("metadata", {})
            if not meta and 'payload' in chunk:
                 meta = chunk['payload'] # Fallback to payload if available
            
            # Check all potential bias keys (Sector, Market Cap, Company)
            # We match values like "Energy", "Small Cap", etc. against keys in bias_map
            
            # Gather relevant values from chunk
            candidates = []
            if "sector" in meta: candidates.append(meta["sector"])
            if "market_cap" in meta: candidates.append(meta["market_cap"])
            if "company" in chunk: candidates.append(chunk["company"])
            
            matched_boost = 1.0
            
            for candidate in candidates:
                if candidate in self.bias_map:
                    # Found a match! Use the boost factor.
                    # If multiple match, we could chain them or take max. Taking max for stability.
                    matched_boost = max(matched_boost, self.bias_map[candidate])
            
            if matched_boost > 1.0:
                 if 'final_score' in chunk:
                    chunk['final_score'] *= matched_boost
                    chunk['bias_boosted'] = True
                    chunk['boost_factor'] = matched_boost
